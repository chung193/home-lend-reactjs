...
import logoWhite from 'enl-images/logo-white.png';
...
function Header(props) {
  ...
  useEffect(() => {
    ...
    console.log(menuMessages.signalOnline.defaultMessage);
    return () => {
      ...
      <Toolbar disableGutters={!open}>
        <div className={cx(classes.brandWrap, dense && classes.dense)}>
          <div>
            ...
            <span>
              <Tooltip className={classes.signalButton} title={signal ? intl.formatMessage(messages.connected) : intl.formatMessage(messages.disconnected)} placement="bottom">
                <IconButton
                  className={classes.ButtonSignal}
                  aria-label="Menu"
                  onClick={console.log('signal button click')}
                  size="large">
                  {signal ? <SignalWifi4BarIcon /> : <SignalWifiStatusbarNullIcon/>}
                </IconButton>
              </Tooltip>
            </span>
          </div>
